...
<!-- <form method="get" action="search.php" style="float:right" class="form-inline" >
  <label for="column" class="sr-only">Select a column to search:</label>
  <select style="margin-right:20px" name="column" class="form-control mb-2 mr-sm-2">
    <option value="forCo">For Company</option>
    <option value="invoiceNum">invoice No.</option>
    <option value="printCo">Print Company</option>
  ...
  <input type="submit" value="Submit" class="btn btn-primary mb-2">
    ...
    if (tdValue == "inProcess") {
      ...
      // Query the database
      $result = mysqli_query($conn, "SELECT * FROM main order by number DESC LIMIT  $num_rows_per_page OFFSET $offset");
      ...
      $total_pages = ceil(mysqli_num_rows(mysqli_query($conn, "SELECT * FROM main")) / $num_rows_per_page);
